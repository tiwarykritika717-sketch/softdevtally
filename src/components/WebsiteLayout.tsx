...
export const WebsiteLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  const navLinks = [
    ...
    { 
      ...
      path: '/courses', 
      hasSub: true,
      subLinks: courseCategories.length > 0 
        ? Array.from(new Set(courseCategories.map(cat => cat.name.trim())))
            .map((name: any) => ({
              name,
              path: `/courses?category=${encodeURIComponent(name)}`
            })).slice(0, 6)
        : [
            { name: 'Financial Accounting', path: `/courses?category=${encodeURIComponent('Financial Accounting')}` },
            { name: 'Software Development', path: `/courses?category=${encodeURIComponent('Software Development')}` },
            { name: 'Office Automation', path: `/courses?category=${encodeURIComponent('Office Automation')}` },
            { name: 'Cyber Security', path: `/courses?category=${encodeURIComponent('Cyber Security')}` }
          ]
    },
    { name: 'GALLERY', path: '/gallery', hasSub: false },
    { 
      name: 'FRANCHISE & COLLABORATION', 
      path: '/franchise-info', 
      hasSub: true,
      subLinks: [
        { name: 'Franchise Process', path: '/franchise-process' },
        { name: 'Partner With Us', path: '/partner-with-us' },
        { name: 'Franchise Enquiry', path: '/franchise-info' }
      ]
    },
  ...
  return (
    <div className="min-h-screen bg-background font-sans selection:bg-blue-100 selection:text-blue-900">
      ...
      {/* Main Header */}
      <header className="sticky top-0 z-50 bg-[#FF8282] text-white shadow-lg">
                  ...
                  <AnimatePresence>
                    {activeDropdown === link.name && (
                      <motion.div 
                        ...
                        exit={{ opacity: 0, y: 10 }}
                        className="absolute top-full left-1/2 -translate-x-1/2 w-64 bg-white border border-gray-100 shadow-2xl rounded-2xl p-4 space-y-1 mt-2 text-[#141414] z-50"
                      >
