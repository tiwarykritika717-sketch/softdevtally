...
export const FranchiseFeeMaster = () => {
  const { franchises, franchiseFees, addFranchiseFee, updateFranchiseFee, deleteFranchiseFee, currentUser } = useApp();
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    // Auto check if a record for this franchiseId already exists
    const existing = franchiseFees.find(ff => ff.franchiseId === formData.franchiseId);

    if (isEditing || existing) {
      const activeId = isEditing || existing!.id;
      updateFranchiseFee(activeId, {
        ...formData,
        franchiseId: formData.franchiseId,
        registrationFees: formData.registrationFees,
        marksheetFees: formData.marksheetFees,
                ...
                <div className="space-y-2">
                  ...
                  <select 
                    ...
                    onChange={(e) => {
                      ...
                      const existing = franchiseFees.find(ff => ff.franchiseId === selId);
                      if (existing) {
                        setIsEditing(existing.id);
                        setFormData({
                          franchiseId: selId,
                          registrationFees: existing.registrationFees,
                          marksheetFees: existing.marksheetFees,
                        ...
                        <td className="px-8 py-6 text-right">
                          <div className="flex items-center justify-end space-x-2">
                            <button 
                              ...
                              className="px-4 py-2 bg-pink-600 text-white rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-pink-700 transition-all shadow-sm"
                            >
                              Edit
                            </button>
                            <button 
                              onClick={() => {
                                if (window.confirm(`Are you sure you want to delete the configuration for ${fee.franchiseName}?`)) {
                                  deleteFranchiseFee(fee.id);
                                }
                              }}
                              className="px-4 py-2 bg-red-600 text-white rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-red-700 transition-all shadow-sm"
                            >
                              Delete
                            </button>
                          </div>
